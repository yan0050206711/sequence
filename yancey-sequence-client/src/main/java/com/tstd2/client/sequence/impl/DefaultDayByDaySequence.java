...
import com.tstd2.client.sequence.task.DataSourcePoolExecutor;
import com.tstd2.client.sequence.task.DayByDaySequenceBufferPoolCheckTask;
import com.tstd2.client.sequence.task.SequenceBufferPoolExecutor;
import com.tstd2.client.sequence.util.Constant;
import com.tstd2.client.sequence.util.DateFormatUtil;
import com.tstd2.client.sequence.util.ShardingOffsetUtil;
...
public class DefaultDayByDaySequence implements DayByDaySequence {
    ...
    public void init() {
        ...
        locks = new ConcurrentHashMap<Integer, Lock>(dataSourceCount);

        for (Map.Entry<Integer, DataSourceManagerFactory.DataSourceInfo> entry : dataSourceManagerFactory.getDataSourcePool().entrySet()) {
    ...
    protected SequenceByDay pollOrderFromQueue(String appName, String currentDayFormat) {
        ...
        if ((sequence = BUFFER_POOL.poll(appName)) != null) {
            ...
            LOGGER.info("队列中还有id，直接返回value，当前的value={}，appName={}", sequence.getAppValue(), appName);
            return sequence;
        }
        return sequence;
    }
    ...
    protected SequenceByDay getOldValue(String appName, String currentDay) {
        ...
        if (sequence == null) {
            ...
            try {
                ...
                if (saveRet == 0) {
                    LOGGER.info("save失败，appName={}, currentDay={}", appName, currentDay);
            ...
            oldSequence = new SequenceByDay(appName, oldValue, currentDay);
            if (LOGGER.isDebugEnabled()) {
